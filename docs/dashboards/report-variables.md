...
Report Variables provide a flexible and powerful way to monitor your metrics and logs in an organized and dynamic manner. This allows you to quickly investigate and troubleshoot issues by applying dynamic filters at the report level.

Let’s say you are monitoring a big number of hosts, some of which are running [Apache](/docs/integration/apache/), some of which are running [MySQL](/docs/integration/mysql/) and the rest of which are running [Elasticsearch](/docs/integration/elasticsearch/). Now suppose that you have a dashboard that displays aggregated information about all these hosts. Assume you were notified of a potential problem with Elasticsearch hosts. Of course, you can reconfigure all dashboard charts to show data only for the Elasticsearch hosts, or filter by them and save that, or create a new report with them. But that is not the most efficient way to handle this, since tomorrow a few more Elasticsearch hosts may be added.

Also consider the above example where you want to group the hosts per [App](/docs/guide/app-guide/) and new Apps may be added from time to time, for example, a few Nginx hosts.
...
**Tag Variables** are used for easy filtering of individual metrics or logs from one or more components using [Tags](/docs/tags/). Instead of having to change each filter value on each [Data Series](/docs/dashboards/chart-builder/#data-source), Tag Variables allow you to set the filters once and then change the values from the Filters menu. 
...
After creating a variable, each [component](/docs/dashboards/components-overview/) needs to be configured so that filtering using Report Variables can be applied. In the example below, we have configured filtering with $os.host and grouping with $groupBy. The “$” sign is used to identify the Report Variables.

![Report Variables Component](/docs/images/dashboards/report-variables-component.png)

Report Variables can be applied to each Data Series in a component.

Note that it is mandatory to configure at least one component with a Report Variable. Otherwise, the Report Variable is of no use, and it will not allow filtering values from the ad-hoc filters drop-down at the top of each dashboard or report. That is an important difference between ad-hoc filters and Report Variables.
